...
from pmd_beamphysics.fields.analysis import accelerating_voltage_and_phase, track_field_1df
...
def fast_autophase_ele(ele, 
                  attribute='theta0_deg', 
                  rel_phase_deg = None,
                  phase_range = None,
                  fmaps=None, t0=0, pz0=0, q0=-1, dz_field=None, mc2=mec2, debug=False):
    """
    ...
    if attribute not in AUTOPHASE_ATTRS:
        raise ValueError(f"attribute '{attribute}' is not allowed, should be one of: {AUTOPHASE_ATTRS}")
    ...
    def phase_f(phase_deg):
        ...
        zf, pzf, tf = track_field_1df(Ez_f, zstop=L, tmax=100*nz/freq, pz0=pz0, t0=t0, q0=q0,
                                     mc2=mc2, max_step = max_step)
...
def fast_autophase_impact(impact_object,
                          settings: dict=None,
                          t0=0,
                          pz0=0.0,
                          full_output=False,
