...
dipole_fieldmap_labels = [
    ...
    'w1', 'w2', # entrance and exit widths?
    'c1', 'c2', 'c3', 'c4', 'c5', 'c6', 'c7', 'c8',  # Enge. c2 = 1/fint will correspond to Bmad. 
    'entrance_s', 'exit_s'
...
def plot_dipole_fieldmap(data, g=None, L=None, ref_data=None, **kwargs):
    ...
    add_line(ax, d['k3'], d['b3'], y0=Yend, p=0.1*np.cos(g*L), linestyle='solid', label='L3')
    add_line(ax, d['k4'], d['b4'], y0=Yend, p=0.1*np.cos(g*L), linestyle='--', label='L4')   
...
def exit_edges(e2=0, w2=0.02, g=0, L=1, s0=0):
    ...
    X = -(1-np.cos(theta))/g
    Z = np.sinc(theta/np.pi) * L + s0  # numpy sinc has a pi in it, so need to remove it
...
def new_dipole_fieldmap_data(L=1, g=0, gamma_ref=1, e1=0, e2=0,  half_gap=0.01, fint=0.5, csr_on=False):
    """
